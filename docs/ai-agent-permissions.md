...
| 操作 | 許可の条件 |
|------|------------|
| RO（参照） | サイト・アプリの利用許可に準じる。既存のサインイン状態での参照も含め、追加の認証利用確認は不要 |
| RW（変更・送信等） | computer-useの対象・操作・範囲を含む計画について、作業開始前にユーザーの承認が必要 |
| 未サインインからの認証 | RO目的でも、対象アカウントと認証を含む計画について事前承認が必要。認証不要の公開ページ参照はRO |

計画段階では、予定する作業手段の実現性を確認し、computer-useが実行中に不用意に追加されない計画にする。RW・認証が必要なら、対象サイト・アプリ、対象アカウント、操作内容・範囲を具体的に提示する。「必要ならcomputer-useを使う」等の包括的な記載では承認条件を満たさない。

実行中に計画外のcomputer-useによるRW・認証が必要になった場合は、計画不備と判断し、該当操作は不許可とする。該当経路を停止して計画不備と作業への影響を報告し、ユーザーの指示を待つ。その場の操作承認や自律的な代替手段への切り替え、計画の改訂・実行で補わない。計画に記載されていても未承認のRW・認証は実行しない。

ツール側の利用許可と、RW・認証を含む計画の事前承認は別である。`allow` / `ask` の設定や操作時の承認だけでは、この計画承認を代替できない。
