...
起点が持つ強みは会話履歴であり、専門知識の優位性ではない。起点は会話の原文と経緯を広めに渡し、parentが依頼を解釈して調査・設計・実装・検証を担う。起点はparentの上司として手順を指示したり、進捗を監視したりしない。

```mermaid
flowchart TB
    U["ユーザー"]
    R["起点：会話履歴の保持・依頼・受け渡し<br/>GPT-6 Luna / xhigh"]
    RA["起点専属の相談役：root-advisor<br/>GPT-6 Sol / medium"]
    A0["起点専属のassistant-L<br/>GPT-6 Luna / low"]
    P["parent：専門案件の担当<br/>調査・設計・実装・検証・報告"]
...
起点は会話の原文を渡し、parentはそれを解釈して業務を進めます。起点がユーザーへ要約するのは、parentから届いた報告です。着手ドラフトと変更経緯、完了時の正式報告を残すことで、会話が圧縮されても当初の理解と結論までの経緯をたどれます。
...
| --- | --- | --- |
| 起点 | 会話履歴から原文と経緯を広めに拾い、専門担当へ依頼する | [起点規約](../dot_codex/common/root-agent.md.tmpl) |
| parent | 依頼を解釈し、専門案件と業務報告を一貫して担う | [共通本文](../.chezmoitemplates/agent/codex/parent.md)、[Sol/Astra用](../.chezmoitemplates/agent/codex/parent-advanced.md)、[広域調査用](../.chezmoitemplates/agent/codex/parent-survey.md) |
...
会話文脈の受け渡しを起点に、専門的な解釈と遂行をparentに置くことで、起点の解釈による制約とメインスレッドの負担を減らす狙いがあります。Luna主体の起点とSol相談役の組み合わせによる品質・消費量の改善は、まだ検証段階です。
