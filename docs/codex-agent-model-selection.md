...
上表のLuna / xhighは `research-worker-L` と `implementation-worker-L` に適用する未実測・可逆な運用仮説である。定型取得の `market-data-worker-L` はmediumを維持する。Terraは階級の中間という理由だけで自動選択しない。Terra xhighが必要な状況ではSol lowも比較候補になるが、同等性は保証されず、現在のロール定義や起動ツールが提供しないeffortへ自動上書きしない。Sol高帯は難しい仕事で維持する。
...
- `collaboration.spawn_agent` の `agent_type` に正式ロールを指定する。`task_name` は `ロール__モデル略称` とし、小文字化してハイフンをアンダースコアに置換する（例: `research_lead__terra`）。これは `nickname_candidates` とは別の名前である。ツール名・引数・通知形式は実行時の仕様に合わせ、同じ履歴・役割・権限・再委任制限を満たす方法へ読み替える。毎回の実装調査は要求せず、制約を守れるか判断できない操作だけ保留する。
