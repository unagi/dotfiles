...
| parentが自処理／workerに実装を委任 | どちらも総成果の設計・実装完了の独立レビューが必要。workerはparentの確認と総成果の設計通過通知を受けて実装する |
| parentがchild成果の設計・作成に関与 | そのparentがchild成果を内部レビューして統合できる。childごとの独立レビュアーは必須でない |
| 全child成果の内部レビューが完了 | それだけで案件を完了にせず、全child成果と結合部分を含む総成果を別個体がレビューする |
| 総成果の独立レビュアーを手配 | 総成果の設計者が手配する。同じロール名でも作成担当とは別個体をレビュー専任・変更禁止で使う |
| childの局所設計で総成果の設計版が変わる | parentが取りまとめて総成果の設計レビューへ戻す。旧版の通過通知で実装を進めない |
...
| parentが結合時に対象を編集 | child個別成果の内部レビューはできるが、総成果の独立レビューは別個体へ依頼する |
