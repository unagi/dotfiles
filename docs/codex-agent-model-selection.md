...
現行方針の確認日は2026-09-25。sourceはサブエージェント19定義（parent 9、worker 7、advisor 2、assistant 1）を管理する。起点モデルを含む`config.toml`はchezmoi管理外である。起点の責務は自身のモデル・effortの認識に依存せず、実環境の設定・配布は別作業である。
...
| 入力・状況 | 期待する対応 |
| --- | --- |
| 軽微な求め／早期回答が困難 | 自身またはassistantで早期回答し、困難ならparentへの委任を確認する。承認済みの委任は再確認しない |
